...
def sample_hiddens(chains: dict[str, Tensor], params: BGRBM, beta: float = 1.0) -> dict[str, Tensor]:
    """Sample h|v for BG-RBM (Gaussian hidden with fixed var = 1/Nv)."""
...
def sample_visibles(chains: dict[str, Tensor], params: BGRBM, beta: float = 1.0) -> dict[str, Tensor]:
    """Sample v|h for BG-RBM (Bernoulli visibles)."""
...
def compute_energy(v: Tensor, h: Tensor, params: BGRBM) -> Tensor:
    """Energy E(v,h) = -a^T v - b^T h - v^T W h + 1/2 h^T Γ h, with Γ = diag(exp(eta))."""
...
def compute_energy_visibles(v: Tensor, params: BGRBM) -> Tensor:
    """Marginalized energy over h; free-energy uses Γ^{-1} = diag(exp(-eta))."""
...
def compute_energy_hiddens(h: Tensor, params: BGRBM) -> Tensor:
    """Energy marginalized over v (constant wrt v), includes Gaussian prior term."""
...
def compute_gradient(
    ...
    """Attach gradients for a, b, W (eta is fixed here and should be ignored by optimizer)."""
    _compute_gradient(
        v_data=data["visible"],
...
def init_chains(
    ...
    """Initialize chains: sample v (uniform or provided), then h|v with fixed variance."""
...
def init_parameters(
    ...
    """Return BGRBM with fixed hidden variance = 1/Nv (precision γ=Nv, eta=log Nv)."""
    data = dataset.data
